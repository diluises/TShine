...
int run_ana(){
	...
	mapKin2D.CheckBinsOverlap(false);
	...
	//Same for the 2D map
	TSPhaseSpaceFld mapKin2D_All("map_2D_All","Events ",mapKin2D.Title(),mapKin2D);
	...
	Mom.ForceToRange(false);
	 ...
	 listOfToFParamFuncs.Add(tofGen.GetParametrization("part",i));
       ...
       //typical width of m2 varies largely with the momentum
       ...
       // Alt. to the auto binning
       // It is possible to propagate range and binning over the phase space
       // Range for a specific variable (dEdx,ToF...)
       // can be specified as a function of a 
       // phase space variable (Theta,Mom,...)
       // using two TGraph's (one for the lower bound
       // and one for the upper bound)
       // Values are interpolated (for out of range 
       // closest given point in the graph if used)
       //it can be applied two times to rebin both axis
       //
       /* Example: (only 3 point for brevity)
       TGraph *g_min = new TGraph();
       g_min->SetPoint(0,1,-0.1);
       g_min->SetPoint(1,3,-0.4);
       g_min->SetPoint(2,10,-5);
       
       TGraph *g_max = new TGraph();
       g_max->SetPoint(0,1,2);
       g_max->SetPoint(1,2,1.3);
       g_max->SetPoint(2,10,5);

       //Bin dEdx axis as functio of the Mom value for the associated phase space
       //volume (calculate at the center of the bin)
       //if numbed of points (3rd argument) is <=0 same value of the target histogram is used.
       //
       //
       
       dataSetMgr2D.SetBinning(dEdx,Mom,100,*g_min,*g_max);

       //opt. limit range if needed
       //dataSetMgr2D.GetPhaseSpaceMap()->SelectRange(Theta,30,200,true);
       //dataSetMgr2D.SetBinning(dEdx,100,*g_min,*g_max);
       //dataSetMgr2D.GetPhaseSpaceMap()->RemoveSelection();
	 ...
	 //These  MapFolder have been created at the beginning
	 //They can be used standalone to have an accupancy map

	 // This should be called for every positive track in the event
	 // here there is just a call per event 
	 mapKin3D_Pos.Fill();

	 //This map will show events per phase space bin (it is called once per event)
	 mapKin2D_All.Fill();

	 // N.B.:There is a MapFolder object also in each DataSetMgr obj which is
         // filled automatically via the Fill(...) method 
	 // Outside this loop it is shown how to retrieve the DaraSetMgr MapFld	 
       }


       //
       // -- Cumulative check histos
       //
       hGenPtot.Write();
       hGenBB.Write();
       ...
       mapKin2D_All.GetHistogramFld()->Write();
       
       
       // -- The standalone DataBin 
       dataBin.GetHistoFolder()->Write();
	 ...
	 dataBin.GetDataTree()->Write();
       ...
       dataSetMgr2D.GetDataBin(2)->GetHistoFolder()->Write();
       ...
       //Get the Map Folder for the 3D Phase Space
       //print each volume with the corresponding entries
       //Write the correlation histos (x-y,x-z,y-z)
       dataSetMgr.GetPhaseSpaceFld()->Print();
       dataSetMgr.GetPhaseSpaceFld()->GetHistogramFld()->At(0)->Write();
       dataSetMgr.GetPhaseSpaceFld()->GetHistogramFld()->At(1)->Write();
       dataSetMgr.GetPhaseSpaceFld()->GetHistogramFld()->At(2)->Write();


       //Write out 2D Phase Space Fld 
	...
	/*
	EDM::DataContainer dataCont("Data","#theta,p_{tot}");

	for(int i=0;i<10;++i){

   		
          dataCont.BuildFolder(Form("theta_%i",i+1));

          for(int j=0; j<5;++j){ 

		dataCont.BuildFolder(Form("theta_%i/ptot_%i",i+1,j+1));	

	        dataCont.Place(g_P_dEdx_Pdf,Form("theta_%i/ptot_%i",i+1,j+1));
	        dataCont.Place(g_Pi_dEdx_Pdf,Form("theta_%i/ptot_%i",i+1,j+1));
	        dataCont.Place(g_K_dEdx_Pdf,Form("theta_%i/ptot_%i",i+1,j+1));
	        for(int k=0;k<4;++k)dataCont.Put(*g_P_dEdx_Pdf,Form("theta_%i/ptot_%i",i+1,j+1));
	...
	*/
	...
	//if last arguments is true a bin will be selected even if
        //it is partially contained in the selection range
	//
	//mapKin2D.SelectRange(Mom,4,8,true);
	//mapKin2D.SelectRange(Theta,10,40,true);


	for(int ivol=1; ivol<=NumOfVols; ++ivol){

	  //this will work of a Range is set
	  if( mapKin2D.HasSelection() && !mapKin2D.VolumeIsSelected(ivol) ) continue;
	
	  // keep one random volume
	  if(ivol!=4) continue;
	...
	mapKin2D.RemoveSelection();



	//
	// ---------- Simple test of usage of Convolution Funcitons
	//
 
	TSVariable conv_x("conv_x","x","conv x");
	conv_x.SetRange(0.,5.);
	TSParameter conv_g_mean("conv_g_mean","gm","conv g mean");
	TSParameter conv_g_sig("conv_g_sig","gs","conv g sig");
	TSParameter conv_e_mean("conv_e_mean","em","conv e mean");
	TSParameter conv_e_tau("conv_e_tau","et","conv e tau");

	conv_g_mean.SetValue(0.);
	conv_g_sig.SetValue(3.);
	conv_e_mean.SetValue(0.);
	conv_e_tau.SetValue(-0.3);

	TSFunc1DExpPdf conv_exp("conv_exp","",conv_x,conv_e_mean,conv_e_tau);
	TSFunc1DGaussPdf conv_gauss("conv_gauss","",conv_x,conv_g_mean,conv_g_sig);

	TSFunc1DConvolution conv_func("conv_func","conv func",conv_x,&conv_exp,&conv_gauss,0.,5.);

	TGraph *conv_ga = conv_func.BuildGraphX(100);

	conv_ga->Write("conv_expga");
