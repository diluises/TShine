...
inline vector<TString> GetPath2(TString path_full){

   std::vector<TString> list;


   int slash_pos = path_full.Last('/');

   TString path_to = path_full(0,slash_pos);

   TString name = path_full(slash_pos+1,path_full.Length()-slash_pos-1);

   list.push_back(path_to);
   list.push_back(name);

   return list;
 }

inline TString GetPath(TString path_full, TString* path_to, TString *name) {

	int slash_pos = path_full.Last('/');

	*path_to = path_full(0,slash_pos);

	*name = path_full(slash_pos+1,path_full.Length()-slash_pos-1);
