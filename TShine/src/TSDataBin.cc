...
:TSNamed( other.Name(), other.GetLabel(), other.Title() )
{
	...
	for(int i=0; i<other.GetNumOfBidimPairs(); ++i){

	  int idx = other.GetBidimVarXIdx(i);
	  int idy = other.GetBidimVarYIdx(i);
	  
	  
	  SetBidimensional(idx,idy);
...
int TSDataBin::GetBidimVarXIdx(int i) const{

  if(i<0 || i>fBidimVarXIdx.size()-1){
    MSG::ERROR(__FILE__,"::",__FUNCTION__," index out of range: ",i);
    return -1;
  }

  
  if(fBidimVarXIdx.size() != fBidimVarYIdx.size()){
    MSG::ERROR(__FILE__,"::",__FUNCTION__," X and Y lists have different dimensions");
    MSG::ERROR(" ",fBidimVarXIdx.size()," and ",fBidimVarYIdx.size());
    return -1;
  }

  if(fBidimVarXIdx.size() != fNumOfBidimPairs){
    MSG::ERROR(__FILE__,"::",__FUNCTION__," index vector size and fNumOfBidimPairs are different");
    MSG::ERROR(" ",fBidimVarXIdx.size()," and ",fNumOfBidimPairs);
    return -1;
  }


  return fBidimVarXIdx[i];

}


int TSDataBin::GetBidimVarYIdx(int i) const{

  if(i<0 || i>fBidimVarYIdx.size()-1){
    MSG::ERROR(__FILE__,"::",__FUNCTION__," index out of range: ",i);
    return -1;
  }
  ...
  if(fBidimVarYIdx.size() != fNumOfBidimPairs){
    MSG::ERROR(__FILE__,"::",__FUNCTION__," index vector size and fNumOfBidimPairs are different");
    MSG::ERROR(" ",fBidimVarXIdx.size()," and ",fNumOfBidimPairs);
    return -1;
  }
...
int TSDataBin::SetBidimensional(TSVariable& var_x, TSVariable& var_y){
	...
	return SetBidimensional(idx,idy);

}


int TSDataBin::SetBidimensional(int idx, int idy){


  if(idx<1 || idx>fNumOfVars){
    MSG::ERROR(__FILE__,"::",__FUNCTION__," Variable X index out of range: ",idx);
    MSG::ERROR(" tot num of vars is ",fNumOfVars);
    return -1;
  }


  if(idy<1 || idy>fNumOfVars){
    MSG::ERROR(__FILE__,"::",__FUNCTION__," Variable Y index out of range: ",idy);
    MSG::ERROR(" tot num of vars is ",fNumOfVars);
    return -1;
  }


  fBidimVarXIdx.push_back(idx);
  fBidimVarYIdx.push_back(idy);
  fNumOfBidimPairs=fBidimVarXIdx.size();
