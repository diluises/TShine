...
int TSPhaseSpaceMap::AddVolume(TSPhaseSpaceVolume& vol) {
	...
	fVolIsSelected[fNumOfVols]=true;
...
void TSPhaseSpaceMap::CheckBinsOverlap(bool b){
...
void TSPhaseSpaceMap::Print() const{
	...
	for(int i=1; i<=GetNumOfVolumes();++i){
	  cout<<"vol: "<<i<<" "<<GetVolume(i)->GetDescription();
	  if(fHasSelection) cout<<" selected: "<<VolumeIsSelected(i);  
	  cout<<endl;
	}
 ...
 */
	...
	for(int ivol=1; ivol<=GetNumOfVolumes(); ++ivol){
	  ...
	  if( fHasSelection ){
	    ...
	    if( !VolumeIsSelected(ivol) ) continue;
	  }
	  ...
	  if( is_sel ){
	    ...
	    //cout<<"is_sel "<<ivol<<" "<<GetNumOfVolumes()<<"; "<<axis_low<<" "<<axis_up<<"; "<<range_min<<" "<<range_max<<endl;
	    ...
	    //cout<<"is_not_sel "<<ivol<<" "<<GetNumOfVolumes()<<"; "<<axis_low<<" "<<axis_up<<"; "<<range_min<<" "<<range_max<<endl;
	  }
